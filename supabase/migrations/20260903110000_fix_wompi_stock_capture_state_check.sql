...
begin
  ...
  if v_order.payment_status = 'paid' then
    if p_wompi_status = 'APPROVED' then
      ...
      select stock_capture_status into v_stock_capture_status
        from public.store_orders
        where id = v_order.id;

      if v_stock_capture_status = 'captured' then
        return query select 'idempotent_paid'::text, 'paid'::text;
      end if;

      if v_stock_capture_status in ('insufficient_stock', 'integrity_exception') then
        return query select 'paid_stock_exception'::text, 'paid'::text;
      end if;

      raise exception 'unexpected stock capture state % (outcome %) for paid order %',
        v_stock_capture_status, v_stock_capture_outcome, v_order.id;
    end if;
