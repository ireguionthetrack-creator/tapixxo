...
-- The primary link is browser-first. Keep the old Google Maps deep link as a
-- secondary fallback so existing data is not discarded.
alter table public.company_google_review_destinations
  add column if not exists google_maps_write_a_review_uri text;

alter table public.company_google_review_destinations
  drop constraint if exists company_google_review_destinations_google_maps_write_a_review_uri_check;

alter table public.company_google_review_destinations
  add constraint company_google_review_destinations_google_maps_write_a_review_uri_check
  check (
    google_maps_write_a_review_uri is null
    or (
      google_maps_write_a_review_uri ~* '^https://([a-z0-9-]+\.)*google\.com/'
      and char_length(google_maps_write_a_review_uri) <= 2000
    )
  );

-- Rewrite current plate destinations before replacing the stored primary URL.
...
-- Existing companies do not have to repeat the setup flow.
update public.company_google_review_destinations as destination
set
  google_maps_write_a_review_uri = coalesce(
...
declare
  ...
  v_place_id text;
  v_primary_review_uri text;
  v_fallback_review_uri text;
begin
  ...
  v_fallback_review_uri := nullif(btrim(p_write_a_review_uri), '');
  if v_fallback_review_uri is not null and (
    v_fallback_review_uri !~* '^https://([a-z0-9-]+\.)*google\.com/'
    or char_length(v_fallback_review_uri) > 2000
  ) then
    raise exception 'invalid Google Maps review URL';
  end if;

  v_primary_review_uri :=
  ...
  on conflict (company_id) do update set
    ...
    google_maps_write_a_review_uri = case
      when current_destination.google_place_id = excluded.google_place_id
        then coalesce(
          excluded.google_maps_write_a_review_uri,
          current_destination.google_maps_write_a_review_uri
        )
      else excluded.google_maps_write_a_review_uri
    end,
    updated_at = now();

  if coalesce(cardinality(p_code_ids), 0) > 0 then
    update public.codes
    set destination_url = v_primary_review_uri
