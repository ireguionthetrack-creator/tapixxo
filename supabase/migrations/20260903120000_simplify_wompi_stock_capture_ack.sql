...
begin
  ...
  if v_order.payment_status = 'paid' then
    if p_wompi_status = 'APPROVED' then
      -- The capture function is atomic and records terminal stock exceptions
      -- on the order. Any unexpected database failure raises and rolls back.
      perform 1
        from public.capture_paid_store_order_stock(v_order.id);
